<template>
  <div class="app min-h-screen overflow-hidden font-sans text-white">
    <SVGContainer />
    <div class="md:px-0 container relative px-2 mx-auto">
      <nav class="sm:pt-6 absolute left-0 right-0 flex items-center justify-between px-2 pt-4">
        ...
        <div class="flex items-center leading-relaxed text-center">
          <template v-if="windowWidth > breakpoints.sm">
            <router-link
              :to="{ name: 'profile'}"
              class="profile hover:text-blue-200 sm:text-base relative pb-1 text-sm text-white border-b border-transparent"
            >Profile</router-link>
          </template>
        </div>
      </nav>
      <div class="app-content flex flex-col items-center justify-center min-h-screen">
        <router-view />
      </div>
    </div>
  </div>
...
.profile.router-link-exact-active::after {
  content: "";
